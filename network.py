...
class MLP(nn.Module):
    def __init__(self, input_dim, layers):
        ...
        self.layers = layers
        self.input = nn.Linear(self.input_dim, layers[0])
...
class DistributionMatching:
    def __init__(self, module):
        self.hook = module.register_forward_hook(self.find_mean_and_var)

    def find_mean_and_var(self, module, input, output):

        mean = input[0].mean([0, 2, 3])
        var = input[0].permute(1, 0, 2, 3).contiguous().view([input[0].shape[1], -1]).var(1, unbiased=False)
        self.src_feat_mean = module.running_mean.data
        self.src_feat_var = module.running_var.data
        self.tgt_feat_mean = mean
        self.tgt_feat_var = var
